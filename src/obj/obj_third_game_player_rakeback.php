...
class obj_third_game_player_rakeback
{
    /**
     * 获取未进行结算的游戏记录统计
     ...
     * @throws
     * */
    public function check_all_player_record_stats(int $Ymd)
    {
        $res = bonjour::$container->get(tgm_player_rakeback_game_record_stats::class)->select_by_ymd('*',$Ymd);
        if($res->code) return $res;
        $res = bonjour::$container->get(tgm_player_rakeback_bill_details::class)->select_by_ymd('*',$Ymd);
        if($res->code) return $res;
    }
}